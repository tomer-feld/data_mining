...
try:
    ...
    for element in elements:
        ...
        # print(soup.prettify())
        tweet_text  = soup.find('div', attrs={'class':'css-901oao r-18jsvk2 r-1qd0xha r-a023e6 r-16dba41 r-rjixqe r-bcqeeo r-bnwqim r-qvutc0'})
        print('Tweet: ',tweet_text.get_text())
        # print(soup)
        tweet_numbers = soup.findAll('div', attrs={'class':'css-1dbjc4n r-xoduu5 r-1udh08x'})
        # print(tweet_numbers)
        # for item in tweet_numbers:
            # print(item.get_text())
        print(f'Comments: {tweet_numbers[0].get_text()}, Retweets: {tweet_numbers[1].get_text()}, Likes: {tweet_numbers[2].get_text()}')
        print()
    print(len(elements))
finally:
    # driver.quit()
